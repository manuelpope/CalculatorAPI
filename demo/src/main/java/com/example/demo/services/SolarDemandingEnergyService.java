...
public class SolarDemandingEnergyService {
    ...
    public int calculateTotalDemandWh() {
        ...
        List<Load> listDC = this.loadList.stream().filter(LOAD_DC_POWER).map(MAPPER_DC).collect(Collectors.toList());


        List<Load> listAC = this.loadList.stream().filter(LOAD_AC_POWER).map(MAPPER_AC).collect(Collectors.toList());
